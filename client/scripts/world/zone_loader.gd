...
func load_zone_snapshot(zone_id: String = "ashen_hollow") -> Dictionary:
	var manifest := _load_json(CONTENT_ROOT + "manifest.json")
	var zone_path := "zones/%s.json" % zone_id
	var zone_data := _load_json(CONTENT_ROOT + zone_path)
	var poi_path := zone_data.get("poi_manifest", "points_of_interest/starter_pois.json")
	var poi_data := _load_json(CONTENT_ROOT + poi_path)
	return {
